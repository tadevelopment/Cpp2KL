/* 
 * This auto-generated file contains typemapping conversion fn
 * declarations for the data types found in Fabric2Arnold codegen file
 *  - Do not modify this file, it will be overwritten
 */


inline bool KLAtUserParamIterator_to_CPAtUserParamIterator(const Fabric::EDK::KL::AtUserParamIterator & from, AtUserParamIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtUserParamIterator_to_KLAtUserParamIterator(AtUserParamIterator & from, const Fabric::EDK::KL::AtUserParamIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool StringArr_to_AtEnum(const Fabric::EDK::KL::VariableArray< Fabric::EDK::KL::String > & from, AtEnum & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtEnum_to_StringArr(AtEnum & from, const Fabric::EDK::KL::VariableArray< Fabric::EDK::KL::String > & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KlAtLicenseInfo_to_AtLicenseInfo(const Fabric::EDK::KL::AtLicenseInfo & from, AtLicenseInfo & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtLicenseInfo_to_KLAtLicenseInfo(AtLicenseInfo & from, const Fabric::EDK::KL::AtLicenseInfo & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool RGB_to_AtRGB(const Fabric::EDK::KL::RGB & from, AtRGB & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtRGB_to_RGB(AtRGB & from, const Fabric::EDK::KL::RGB & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtBucket_to_CPAtBucket(const Fabric::EDK::KL::AtBucket & from, AtBucket & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtBucket_to_KLAtBucket(AtBucket & from, const Fabric::EDK::KL::AtBucket & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Float32_to_float(const Fabric::EDK::KL::Float32 & from, float & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool float_to_Float32(float & from, const Fabric::EDK::KL::Float32 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Boolean_to_bool(const Fabric::EDK::KL::Boolean & from, bool & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool bool_to_Boolean(bool & from, const Fabric::EDK::KL::Boolean & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtAOVIterator_to_CPAtAOVIterator(const Fabric::EDK::KL::AtAOVIterator & from, AtAOVIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtAOVIterator_to_KLAtAOVIterator(AtAOVIterator & from, const Fabric::EDK::KL::AtAOVIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtTextureHandle_to_CPAtTextureHandle(const Fabric::EDK::KL::AtTextureHandle & from, AtTextureHandle & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtTextureHandle_to_KLAtTextureHandle(AtTextureHandle & from, const Fabric::EDK::KL::AtTextureHandle & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtNodeEntry_to_CPAtNodeEntry(const Fabric::EDK::KL::AtNodeEntry & from, AtNodeEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtNodeEntry_to_KLAtNodeEntry(AtNodeEntry & from, const Fabric::EDK::KL::AtNodeEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool String_to_char(const Fabric::EDK::KL::String & from, char* & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool char_to_String(char* & from, const Fabric::EDK::KL::String & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool SInt64_to_long_long(const Fabric::EDK::KL::SInt64 & from, long long & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool long_long_to_SInt64(long long & from, const Fabric::EDK::KL::SInt64 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtParamIterator_to_CPAtParamIterator(const Fabric::EDK::KL::AtParamIterator & from, AtParamIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtParamIterator_to_KLAtParamIterator(AtParamIterator & from, const Fabric::EDK::KL::AtParamIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KlAtRay_to_AtRay(const Fabric::EDK::KL::AtRay & from, AtRay & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtRay_to_KLAtRay(AtRay & from, const Fabric::EDK::KL::AtRay & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtNodeEntryIterator_to_CPAtNodeEntryIterator(const Fabric::EDK::KL::AtNodeEntryIterator & from, AtNodeEntryIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtNodeEntryIterator_to_KLAtNodeEntryIterator(AtNodeEntryIterator & from, const Fabric::EDK::KL::AtNodeEntryIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool UInt16_to_AtUInt16(const Fabric::EDK::KL::UInt16 & from, AtUInt16 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtUInt16_to_UInt16(AtUInt16 & from, const Fabric::EDK::KL::UInt16 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KlAtMetaDataEntry_to_AtMetaDataEntry(const Fabric::EDK::KL::AtMetaDataEntry & from, AtMetaDataEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtMetaDataEntry_to_KLAtMetaDataEntry(AtMetaDataEntry & from, const Fabric::EDK::KL::AtMetaDataEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtMetaDataStore_to_CPAtMetaDataStore(const Fabric::EDK::KL::AtMetaDataStore & from, AtMetaDataStore & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtMetaDataStore_to_KLAtMetaDataStore(AtMetaDataStore & from, const Fabric::EDK::KL::AtMetaDataStore & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Data_to_void(const Fabric::EDK::KL::Data & from, void* & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool void_to_Data(void* & from, const Fabric::EDK::KL::Data & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool UInt32_to_AtUInt32(const Fabric::EDK::KL::UInt32 & from, AtUInt32 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtUInt32_to_UInt32(AtUInt32 & from, const Fabric::EDK::KL::UInt32 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Box3_to_AtBBox(const Fabric::EDK::KL::Box3 & from, AtBBox & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtBBox_to_Box3(AtBBox & from, const Fabric::EDK::KL::Box3 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KlParamValue_to_AtParamValue(const Fabric::EDK::KL::AtParamValue & from, AtParamValue & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtParamValue_to_KLParamValue(AtParamValue & from, const Fabric::EDK::KL::AtParamValue & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtNodeMethods_to_CPAtNodeMethods(const Fabric::EDK::KL::AtNodeMethods & from, AtNodeMethods & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtNodeMethods_to_KLAtNodeMethods(AtNodeMethods & from, const Fabric::EDK::KL::AtNodeMethods & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Float64_to_double(const Fabric::EDK::KL::Float64 & from, double & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool double_to_Float64(double & from, const Fabric::EDK::KL::Float64 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtScrSample_to_CPAtScrSample(const Fabric::EDK::KL::AtScrSample & from, AtScrSample & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtScrSample_to_KLAtScrSample(AtScrSample & from, const Fabric::EDK::KL::AtScrSample & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KlAtTextureParams_to_AtTextureParams(const Fabric::EDK::KL::AtTextureParams & from, AtArray & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtTextureParams_to_KLAtTextureParams(AtArray & from, const Fabric::EDK::KL::AtTextureParams & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool UInt8_to_AtByte(const Fabric::EDK::KL::UInt8 & from, AtByte & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtByte_to_UInt8(AtByte & from, const Fabric::EDK::KL::UInt8 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtNodeIterator_to_CPAtNodeIterator(const Fabric::EDK::KL::AtNodeIterator & from, AtNodeIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtNodeIterator_to_KLAtNodeIterator(AtNodeIterator & from, const Fabric::EDK::KL::AtNodeIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtMetaDataIterator_to_CPAtMetaDataIterator(const Fabric::EDK::KL::AtMetaDataIterator & from, AtMetaDataIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtMetaDataIterator_to_KLAtMetaDataIterator(AtMetaDataIterator & from, const Fabric::EDK::KL::AtMetaDataIterator & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KlArray_to_AtArray(const Fabric::EDK::KL::AtArray & from, AtArray & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtArray_to_KLArray(AtArray & from, const Fabric::EDK::KL::AtArray & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtUserParamEntry_to_CPAtUserParamEntry(const Fabric::EDK::KL::AtUserParamEntry & from, AtUserParamEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtUserParamEntry_to_KLAtUserParamEntry(AtUserParamEntry & from, const Fabric::EDK::KL::AtUserParamEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool UInt64_to_AtUInt64(const Fabric::EDK::KL::UInt64 & from, AtUInt64 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtUInt64_to_UInt64(AtUInt64 & from, const Fabric::EDK::KL::UInt64 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Mat44_to_AtMatrix(const Fabric::EDK::KL::Mat44 & from, AtMatrix & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtMatrix_to_Mat44(AtMatrix & from, const Fabric::EDK::KL::Mat44 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Color_to_AtRGBA(const Fabric::EDK::KL::Color & from, AtRGBA & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtRGBA_to_Color(AtRGBA & from, const Fabric::EDK::KL::Color & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtNode_to_CPAtNode(const Fabric::EDK::KL::AtNode & from, AtNode & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtNode_to_KLAtNode(AtNode & from, const Fabric::EDK::KL::AtNode & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtParamEntry_to_CPAtParamEntry(const Fabric::EDK::KL::AtParamEntry & from, AtParamEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtParamEntry_to_KLAtParamEntry(AtParamEntry & from, const Fabric::EDK::KL::AtParamEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KlAtAOVEntry_to_AtAOVEntry(const Fabric::EDK::KL::AtAOVEntry & from, AtAOVEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtAOVEntry_to_KLAtAOVEntry(AtAOVEntry & from, const Fabric::EDK::KL::AtAOVEntry & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Vec2_to_AtPoint2(const Fabric::EDK::KL::Vec2 & from, AtPoint2 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtPoint2_to_Vec2(AtPoint2 & from, const Fabric::EDK::KL::Vec2 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool Vec3_to_AtPoint(const Fabric::EDK::KL::Vec3 & from, AtPoint & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool AtPoint_to_Vec3(AtPoint & from, const Fabric::EDK::KL::Vec3 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool SInt32_to_int(const Fabric::EDK::KL::SInt32 & from, int & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool int_to_SInt32(int & from, const Fabric::EDK::KL::SInt32 & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtList_to_CPAtList(const Fabric::EDK::KL::AtList & from, AtList & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtList_to_KLAtList(AtList & from, const Fabric::EDK::KL::AtList & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool KLAtShaderGlobals_to_CPAtShaderGlobals(const Fabric::EDK::KL::AtShaderGlobals & from, AtShaderGlobals & to) {
  #pragma message("Implement Me")
  return true; 
}

inline bool CPAtShaderGlobals_to_KLAtShaderGlobals(AtShaderGlobals & from, const Fabric::EDK::KL::AtShaderGlobals & to) {
  #pragma message("Implement Me")
  return true; 
}

