...
def _is_param_array(arg_name, cpp_type, next_arg):
    if next_arg:
        ...
        if array_conv and array_conv[0] == cpp_type.replace(" ", ""):
...
def process_function(functionNode, class_name=''):
    ...
    cpp_fn_imp = fn_name + '('
    if class_name:
        cpp_fn_imp = _param_name('this_') + '->' + cpp_fn_imp
    ...
    if kl_returns:
        ...
        if kl_base_returns in kl_pod_types:
            cpp_fn_imp = '  %s %s = %s;\n  Fabric::EDK::KL::%s _result;\n  %s(%s, _result);\n' % (cpp_returns, res, cpp_fn_imp, kl_returns, to_fn, res)
        else:
            cpp_fn_imp = '  %s %s = %s;\n  %s(%s, _result);' % (cpp_returns, res, cpp_fn_imp, to_fn, res)
    else:
        cpp_fn_imp = '  %s;\n' % cpp_fn_imp
